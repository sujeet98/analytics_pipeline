...
with src as (
  ...
  from `sujeet_data_analytics_workspace`.`bronze_dev`.`inventory_items`
    ...
    where ingest_ts_utc >= dateadd(day, -2, (select coalesce(max(ingest_ts_utc), '1900-01-01') from sujeet_data_analytics_workspace.silver_dev.stg_look__inventory_items))
